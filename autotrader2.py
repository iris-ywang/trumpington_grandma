...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.bids = set()
        self.on_hold_bid_prices = {}
        self.asks = set()
        self.on_hold_ask_prices = {}
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            n_ave = 5
            self.mid_history.append((bid_prices[0] + ask_prices[0]) / 2) # new
            # if len(self.mid_history > n_ave): self.mid_history = self.mid_history[1:] # new ??? why
            new_bid_price = bid_prices[0]-100 if bid_prices[0] != 0 else 0
            new_ask_price = ask_prices[0]+100 if ask_prices[0] != 0 else 0

            # If new price is not one of the bid price (i.e. not limit order), AND it's not my
            # first bid, I will cancel my (market) order, and my bid ID return to 0
    ...
    def on_order_status_message(self, client_order_id: int, fill_volume: int, remaining_volume: int,
        ...
        if remaining_volume == 0:
            ...
            if client_order_id in self.on_hold_bid_prices:
                del self.on_hold_bid_prices[client_order_id]

            self.asks.discard(client_order_id)
            if client_order_id in self.on_hold_ask_prices:
                del self.on_hold_ask_prices[client_order_id]
    ...
    @staticmethod
    def gradient_by_moving_average(lst, n_ave=5):
        n_points = len(lst)
        if n_points <= n_ave:
            n_ave = n_points
