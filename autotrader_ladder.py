...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            #         self.asks.pop(self.ask_prices[price])
            #         to_delete.append(price)
            # for price in to_delete: self.ask_prices.pop(price,0)
            to_delete = []
            ...
            #         self.bids.pop(self.bid_prices[price]) # leave this to status update
            #         to_delete.append(price)
            # for price in to_delete: self.bid_prices.pop(price,0)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids.keys():
            ...
            self.logger.warn("Unable to hedge order %d.", client_order_id)
